...
hist_sex<-ggplot(d, aes(p_cor_sex)) + 
  ...
  theme(plot.caption = ggtext::element_markdown(hjust = 0, size = 12, family = "Times New Roman"), axis.title = element_text(size = 14), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), plot.margin = unit(c(1,1,1,1),"lines"))

# save histogram to file
#png("paper/figures/hist_sex.png") # make file
jpeg("paper/figures/hist_sex.jpeg", quality = 100, res = 300, width = 1400, height = 1400) # make file
...
hist_language<-ggplot(d, aes(p_cor_lang)) + geom_histogram(bins = 10) +
  ...
  #                        80))+
  theme_bw()+
  theme(plot.caption = ggtext::element_markdown(hjust = 0, size = 12), axis.title = element_text(size = 14), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), plot.margin = unit(c(1,1,1,1),"lines"))


# save histogram to file
#png("paper/figures/hist_language.png")
jpeg("paper/figures/hist_language.jpeg", quality = 100, res = 300, width = 1400, height = 1400)
print(hist_language)
dev.off()
...
jpeg("paper/figures/hist_age.jpeg", quality = 100, res = 300, width = 1400, height = 1400)
print(hist_age)
dev.off()