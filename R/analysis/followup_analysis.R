...
for(i in 1:length(unique(data$subject))){
  ...
  if(is.na(childcare)==TRUE){
    ###
    #print(extract_json("Childcare", survey_data)[[1]])
    #print(strsplit(extract_json("Childcare", survey_data)[[1]],split = " ")[[1]])
    childcare_txt<-strsplit(extract_json("Childcare", survey_data)[[1]],split = " ")
    #print(childcare)
    ###
    if(length(childcare_txt[[1]])==1){
      #print(strsplit(extract_json("Childcare", survey_data)[[1]],split = "-"))
      childcare_txt<-strsplit(extract_json("Childcare", survey_data)[[1]],split = "-")
