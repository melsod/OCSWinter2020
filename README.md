...
- Consent and debreifing forms, written in HTML code, is found in the [forms folder](forms)
...
- Files in the [data](data) folder:
  ...
  - [final_data](data/final_data.json): The final data collected from 626 participants. File size is 105.5 MB so uses Git LFS in order to be uploaded to GitHub. May cause problems for users unfamiliar with that process.
...
- You may notice that we have included three lines of source code rather than Firebase's one in the code that they provide you. All three of these are needed for our procedure. Do not delete or overwrite them. You may update the version number (e.g., from 6.3.4 to 7.13.2) but this is probably not needed.
